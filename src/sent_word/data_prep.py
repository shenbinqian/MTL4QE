...
def load_and_cache_examples(raw_df, args, tokenizer, sent_scores, no_cache=False, training=True):
    ...
    if os.path.exists(cached_features_file) and (
        ...
        all_label_ids = torch.tensor([f.label_ids for f in features], dtype=torch.int64)
        all_segment_ids = torch.tensor([f.segment_ids for f in features], dtype=torch.int64)

        dataset = TensorDataset(all_input_ids, all_input_mask, all_sent_scores, all_label_ids, all_segment_ids)
