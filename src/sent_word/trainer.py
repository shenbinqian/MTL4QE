...
class Trainer:
    ...
    def _validate(self, dev_dataloader, model, device, loss_type, statsRecord):
        ...
        for batch in dev_dataloader:
            ...
            b_segment_ids = batch[4].to(device)

            with torch.no_grad():
            
                loss, _, sent_out, word_out = model(b_input_ids, b_input_mask, b_sent_score, b_word_label, b_segment_ids, loss_type=loss_type, evaluate=True)
        ...
        total_p_correlation = pearsonr(np.array(sent_presictions).squeeze(), np.array(sent_scores).squeeze())
        total_s_correlation = spearmanr(np.array(sent_presictions).squeeze(), np.array(sent_scores).squeeze())
    ...
    def train(self, train_dataloader, dev_dataloader, model, epochs, optimizer, device, loss_type, validate=True, scheduler=None, output="model.pt"):
        ...
        loss_weights = open(TEMP_DIRECTORY + 'loss_weights_sent-word.txt', 'a')
        ...
        for epoch_i in range(0, epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                loss, weights, _, _ = model(b_input_ids, b_input_mask, b_sent_score, b_word_label, b_segment_ids, loss_type=loss_type)
                ...
                '''
                loss_weights.write(str(weights) + "\n")
    ...
    def predict(self, test_dataloader, model, device, loss_type, out_name="predictions.csv", metrics_name="metrics.txt"):
        ...
        sent_preds = [un_fit(sent.reshape(-1,1)) for sent in sent_presictions]
        df = pd.DataFrame({"sent_score": sent_preds, "word_tag_preds": word_pred_lines, "word_tag_labels": word_label_lines})
        df.to_csv(TEMP_DIRECTORY + out_name, index=False)

        return total_p_correlation[0], total_s_correlation[0], total_f1, total_precision, total_recall
