...
class ExtendedHead(torch.nn.Module):
    ...
    def combine_loss(self, sent_out, emo_out, sent_scores, emo_labels, alpha=0.7, beta=0.3, loss_type=None, evaluate=False):
        # sentence score loss
        ...
        if evaluate:
            return sent_loss + emo_loss, [0, 0]
        else:
            # return the combined loss
            if loss_type:
                # nash loss
                shared_parameters = list(self.pretrained_model.parameters())
                task_specific_params = {"sent_head": self.sent_head, "emo_head": self.emo_head}
                mtl_losses, weights = mtl_loss([sent_loss, emo_loss], shared_parameters, task_specific_params, n_tasks=2, device=torch.device("cuda"), loss_type=loss_type)
                loss = torch.tensor(mtl_losses, requires_grad=True)
                return loss, weights
            else:
                return alpha * sent_loss + beta * emo_loss, [alpha, beta]


    def forward(self, input_ids, attention_mask, sent_scores, emo_labels, token_type_ids=None, alpha=0.7, beta=0.3, loss_type=None, evaluate=False, **kwargs):
        ...
        loss, weights = self.combine_loss(sent_out, emo_out, sent_scores, emo_labels, alpha, beta, loss_type=loss_type, evaluate=evaluate)
        
        return loss, weights, sent_out, emo_out
