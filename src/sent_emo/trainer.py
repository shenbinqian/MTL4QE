...
class Trainer:
    ...
    def _validate(self, dev_dataloader, model, device, loss_type, statsRecord):
        ...
        for batch in dev_dataloader:
            ...
            b_emo_label = batch[3].to(device)
            b_segment_ids = batch[4].to(device)

            with torch.no_grad():
            
                loss, _, sent_out, emo_out = model(b_input_ids, b_input_mask, b_sent_score, b_emo_label, b_segment_ids, loss_type=loss_type, evaluate=True)
    ...
    def train(self, train_dataloader, dev_dataloader, model, epochs, optimizer, device, loss_type, validate=True, scheduler=None, output="model.pt"):
        ...
        loss_weights = open(TEMP_DIRECTORY + 'loss_weights_sent-emo.txt', 'a')
        ...
        for epoch_i in range(0, epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                loss, weights, _, _ = model(b_input_ids, b_input_mask, b_sent_score, b_emo_label, b_segment_ids, loss_type=loss_type)
    ...
    def predict(self, test_dataloader, model, device, loss_type, out_name="predictions.csv", metrics_name="metrics.txt"):
        ...
        df = pd.DataFrame({"sent_score": sent_preds, "emo_preds":emo_predictions})
        df.to_csv(TEMP_DIRECTORY + out_name, index=False)

        return (total_p_correlation[0], total_s_correlation[0]), (total_emo_f1, total_emo_precision, total_emo_recall)
