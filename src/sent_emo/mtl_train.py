...
from src.sent_emo.data_prep import load_and_cache_examples, read_sent_scores, read_emo_labels
from src.common_utils import fit, reader
from src.sent_emo.trainer import Trainer, initialize_optimizer, scheduler
...
from src.sent_emo.mtl_modules import ExtendedHead
...
def main():
    ...
    for i in range(args["n_fold"]):
        ...
        sent, emo = trainer_mtl.predict(test_dataloader, model, device, args["loss_type"], out_name="fold_{}_predictions.csv".format(str(i)), metrics_name="fold_{}_metrics.txt".format(str(i)))
